...
function landerpage() {
	...
	ClToActBx = `
		...
		<div style="width: 100%; background-color: #fff; border: 0px solid red; display: inline-block; position: relative; box-sizing: border-box; padding-bottom: 5px; padding-top: 5px;" align="center">
		...
		<!-- Call To Action -->	
		<div style="float: left; display: inline-block; position: relative; border: 0px solid blue; width: 100%;">
			<div class="CTACnt">		
				<div align="center">
	...
	document.getElementById("TemplateContainer").insertAdjacentHTML("beforeend", centralcolumnhead);
		...
		document.getElementById("CentralColumn").insertAdjacentHTML("beforeend", LanderTemp);
			...
			document.getElementById("LndrCnt").insertAdjacentHTML("afterbegin", MonoDemo);
		
				document.getElementById("MonoDemo").insertAdjacentHTML("afterbegin", WItms);
				document.getElementById("MonoDemo").insertAdjacentHTML("afterbegin", Carousel);
	...
	setTimeout(CTADsp, ((CarPstTtl*loadincre)+1000));
...
function CTADspPsts(j) {
	//alert(CarPstTtl);
	RndPst = Math.floor(Math.random() * CarPstTtl);
	...
	DsppId = window["p".concat(PstArr[PstN])]; 
	DsppId.SetPost(); //Load in repetetive object values
	Dsppno = DsppId.No;
	//alert(Dsppno);
	
	if (DsppId.AnnotType == "Annot") {
		var DspPstImgSrcX = "p".concat(PstArr[PstN], "/monop", PstArr[PstN], "-wi0.svg");
	} else {
		var DspPstImgSrcX = "graphics/HlightBox.svg"
	}
	...
	DspImgBlkX = "DspImgBlk".concat(PstArr[PstN]);
	DspGrZi = DspGrZi + 10;
	...
	if (LdN == 4) {
		//document.getElementById(CrdDspImgId).src = p001[0].imgfile;
		CrdAgleCls = "";
	} 
	
	var CTADsp = `
	<div style="position: absolute; top: 0%; z-index: ${DspGrZi};" class="${CrdAgleCls}">
		<div style="background-color: ${PImgNvBck}; border: 1px solid #ccc; line-height:0;">
			<div style="position: relative; width: 100%;" id=${DspImgBlkX}>
				...
				<div style="position: absolute; width: 100%;">
					<img src=${DspPstImgSrcX} width="100%">	
				</div>
				
				<svg viewBox="0 0 1024 768" style="width: 100%; position: relative;" id=${DspPstBtnBxpX}>	
					<!-- Buttons Loaded here -->
				</svg>
	...
	document.getElementById("CTABxDsp").insertAdjacentHTML("beforeend", CTADsp);
	...
	//Load procedure for Annotated Type Images
		if (DsppId.AnnotType == "Annot") {
			...
			P00XId1VarHead = "DspBtnContainer".concat("P", Dsppno, "BtnID");
			P00XId24VarHead = "DspP".concat(Dsppno, "BtnImgId");
			P00XId3VarHead = "DspP".concat(Dsppno, "DigitPath");
			P00XId5VarHead = "DspAnnotDivLn-p".concat(Dsppno, "-ID");
			P00XId6VarHead = "DspAnnotDivLnPos-P".concat(Dsppno, "-ID");
			...
			for(var j = 1; j < 9; ++j) {
				...
				DspBtnTempX = "DspBtnTemp".concat(j);
				window[DspBtnTempX] = DspBtnTemp;

			}
			
			P00XBtnImgId0T = "P".concat(Dsppno, "BtnImgId0T");
			
			var DspBtn0bTemp = `		
			<rect width="1024" height="768" fill="transparent" id=${P00XBtnImgId0T} />
			`;
			
			PDspBtns = DspBtn0bTemp.concat(DspBtnTemp1, DspBtnTemp2, DspBtnTemp3, DspBtnTemp4, DspBtnTemp5, DspBtnTemp6, DspBtnTemp7, DspBtnTemp8);
			//Load in Btns
			document.getElementById(DspPstBtnBxpX).insertAdjacentHTML("beforeend", PDspBtns);
			...
			for(var i = 1; i < 9; ++i) {
				...
				document.getElementById(AnnotDivLnP00XIDX).setAttribute('x', DsppId[i].LnContPosLeft);
				document.getElementById(AnnotDivLnP00XIDX).setAttribute('y', DsppId[i].LnContPosTop);
				document.getElementById(BtnContainerP00XBtnIDX).setAttribute('x', DsppId[i].BtnPosLeft);
				document.getElementById(BtnContainerP00XBtnIDX).setAttribute('y', DsppId[i].BtnPosTop);
				document.getElementById(AnnotDivLnPosP00XIDX).setAttribute('x1', DsppId[i].LnPosX1);
				document.getElementById(AnnotDivLnPosP00XIDX).setAttribute('y1', DsppId[i].LnPosY1);
				document.getElementById(AnnotDivLnPosP00XIDX).setAttribute('x2', DsppId[i].LnPosX2);
				document.getElementById(AnnotDivLnPosP00XIDX).setAttribute('y2', DsppId[i].LnPosY2);
...
function CarDtaLd() {
	...
	if (pId[0]["PoSp2"][localStorage.getItem("Lang1")][localStorage.getItem("Lang2")] == "") {
		...
		document.getElementById("L2POfSp2WIX").className = "L2WordTypeBlnkLndr";
	} else {
	...
	if (pId[0]["PoSp3"][localStorage.getItem("Lang1")][localStorage.getItem("Lang2")] == "") {
		...
		document.getElementById("L2POfSp3WIX").className = "L2WordTypeBlnkLndr";
	} else {
	...
	document.getElementById("SemTag2WIX").className = "SemTags tag_blanklndr";
	...
	document.getElementById("SemTag3WIX").className = "SemTags tag_blanklndr";
