...
function landerpage() {
	...
	lselstr = {1: L1, 2: L2};
	HdrFtrTxt(L1);//Header and footer text

	LndrSeq(1);

}

function LndrSeq(step) {
	if (step == 1) {
		loadincre = 50;
		LdN = 0;
...
function RndPstArrGen() {
	...
	//Check if randomly generated pos number is present in the array
	if (PstArr[RndPst] != RndPstArr[0] && PstArr[RndPst] != RndPstArr[1] && PstArr[RndPst] != RndPstArr[2] && PstArr[RndPst] != RndPstArr[3] && PstArr[RndPst] != RndPstArr[4]) {
		...
		//Generate different random post number
		RndPstArrGen();
	}
...
function LSel(ln, ll, lssrc) {
	...
	//Fade opacity to 0 of cover square (CSS fade is set at 2s). Strangely this works with a 0 millisecond delay
	setTimeout(function() { revealer(LXImgCvr) }, 0);
